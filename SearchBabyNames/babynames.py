...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name not in name_data:
        ...
        yr_n_rank = name_data[name]
        if year not in yr_n_rank:
            yr_n_rank[year] = rank
            name_data[name] = yr_n_rank
        elif int(rank) < int(yr_n_rank[year]):
            yr_n_rank[year] = rank
...
def read_files(filenames):
    ...
    """
    name_data = {}  # obj, unlimited to stack frame, same memory address
...
def search_names(name_data, target):
    ...
    for name, _ in name_data.items():
        for i in range(len(name)-len(target)+1):
            if target == name[i:i+len(target)].lower():
...
def main():
    ...
    names = read_files(filenames)  # return name_data
